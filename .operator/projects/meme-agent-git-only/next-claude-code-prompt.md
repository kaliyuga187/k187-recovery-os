# Next Claude Code prompt for: meme-agent-git-only

Copy-paste this into Claude Code. Replace the placeholders in {{ }} with the real values from your environment.

```
You are working in repo: C:\Users\nk187\Downloads\Projects\meme-agent-git-only

Goal:
Audit meme-agent-git-only, decide finish-or-archive, then execute that path.

Context:
meme-agent-git-only is a ai-agent project at 0/100 completion, 0/100 deploy readiness, 50/100 health. Stack: unknown. 
...
Files to inspect:
- `C:\Users\nk187\Downloads\Projects\meme-agent-git-only/README.md` (if exists)
- `C:\Users\nk187\Downloads\Projects\meme-agent-git-only/package.json` (if exists)
- `C:\Users\nk187\Downloads\Projects\meme-agent-git-only/Dockerfile` (if exists)
